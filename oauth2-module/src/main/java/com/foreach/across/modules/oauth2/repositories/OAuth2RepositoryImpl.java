...
public class OAuth2RepositoryImpl implements OAuth2Repository {
    ...
    public Collection<OAuth2Client> getOAuth2Clients() {
        return (Collection<OAuth2Client>) sessionFactory.getCurrentSession().createCriteria( OAuth2Client.class ).setResultTransformer(
    ...
    public Collection<OAuth2Scope> getOAuth2Scopes() {
        return (Collection<OAuth2Scope>) sessionFactory.getCurrentSession().createCriteria( OAuth2Scope.class ).setResultTransformer(
                Criteria.DISTINCT_ROOT_ENTITY ).list();
    }
